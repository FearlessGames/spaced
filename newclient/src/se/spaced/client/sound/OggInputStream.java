...
public class OggInputStream extends FilterInputStream {
	...
	private int conversionBufferSize = 4096 * 2;

	// Conversion buffer
	private final byte[] convbuffer = new byte[conversionBufferSize];
	 ...
	 * @param len the maximum number of bytes read.
	 * @return the total number of bytes read into the buffer, or -1 if there is
	 *         n o more data because the end of the stream has been reached.
	 */
	...
	private void initVorbis() throws Exception {
		...
		conversionBufferSize = 4096 / info.channels;
	...
	private int decodePacket(Packet packet) {
		...
		while ((samples = dspState.synthesis_pcmout(pcm, index)) > 0) {
			...
			int bout = (samples < conversionBufferSize ? samples : conversionBufferSize);
