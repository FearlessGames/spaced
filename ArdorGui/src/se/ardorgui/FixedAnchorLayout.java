 ...
 * </p>
 *
 * @see com.ardor3d.extension.ui.layout.AnchorLayoutData
 */
public class FixedAnchorLayout extends UILayout {

	/** map used to track anchor relationship during layout. */
	private final Map<UIComponent, AnchorRecord> _records = Maps.newLinkedHashMap();
	...
	public void layoutContents(final UIContainer container) {
		...
		while (!_records.isEmpty()) {
			Object[] components = _records.keySet().toArray();
			visit((UIComponent) components[components.length - 1]);
			removeVisited();
		}
		...
		if (aRecord != null) {
			...
			for (int x = aRecord.dependants.size(); --x >= 0;) {
				...
				if (dep.getLayoutData() instanceof AnchorLayoutData) {
					...
					FixedAnchorLayout.applyAnchor(dep, (AnchorLayoutData) dep.getLayoutData());
	...
	private static void applyAnchor(final UIComponent comp, final AnchorLayoutData layData) {
		final Vector2 offsetsA = FixedAnchorLayout.getOffsets(comp, layData.getMyPoint(), null);
		final Vector2 offsetsB = FixedAnchorLayout.getOffsets(layData.getParent(), layData.getParentPoint(), null);
